...
TIES（Trim, Elect Sign & Merge）旨在解决多模型合并时的参数冲突和冗余。其核心流程如下：

1. **任务向量**  
   \(\tau*t = \theta*{\mathrm{ft}} - \theta\_{\mathrm{init}}\)

2. **修剪（Trimming）**  
   保留每个 \(\tau_t\) 中幅度最大的前 \(k\) 个参数，其余置零，得到 \(\hat{\tau}\_t\)

3. **选择符号（Elect Sign）**  
   对每个参数，根据所有修剪后向量中的正负号总幅度确定主导方向

4. **不相交合并（Disjoint Merge）**  
   仅对与主导方向一致的参数进行平均，最终参数：
   \[
   \theta*m = \theta*{\mathrm{init}} + \lambda\,\tau_m
   \]
   其中 \(\lambda\) 是缩放因子
...
DARE（Drop And REscale）通过减少 delta 参数的冗余，实现更高效的融合。主要步骤如下：

1. **计算 delta**  
   \(\Delta*t = W*{\mathrm{ft}} - W\_{\mathrm{pre}}\)
...
3. **加回预训练权重**  
   \[
   W*{\mathrm{DARE}} = W*{\mathrm{pre}} + \hat{\Delta}\_t
   \]

**扩展**

- 可与 TIES 结合（dare_ties）或单纯线性方案（dare_linear）
